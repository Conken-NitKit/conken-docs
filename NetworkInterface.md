...
また、コンピュータとネットワークを繋ぐ玄関口となるのが **ネットワークインターフェースカード(NIC)** という機器です。LANカードやネットワークアダプタとも呼ばれます。
...
MACアドレスは、各コンピュータの **NIC** に割り振られており、データリンク層では、この固有の番号を使って **機器を特定** します。
...
- 物理層では **ビット列を信号に変換** する。
- NICは **コンピュータとネットワークを繋ぐ玄関口** の役割を果たす機器。
